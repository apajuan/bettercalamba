...
async function handleGetQueue(context: {
  ...
  try {
    ...
    const results = await env.BETTERCALAMBA_DB.prepare(sql).bind(limit).all();
...
async function handlePermanentDelete(context: {
  ...
  try {
    ...
    const authorCount = await env.BETTERCALAMBA_DB.prepare(
    ...
    await env.BETTERCALAMBA_DB.prepare(`DELETE FROM persons WHERE id = ?1`)
      .bind(person_id)
      .run();
