...
- [ ] **Add City College of Calamba to `departments.json`** — 21 services carry `officeSlug: "city-college-of-calamba"`, but there is no matching entry in `src/data/directory/departments.json`, so those services link to a department page that does not exist. Every other `officeSlug` in the data (19 of 20) resolves correctly
